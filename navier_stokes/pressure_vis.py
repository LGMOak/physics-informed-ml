...
def visualise():
    try:
        # Expected order: t_mean, t_std, x_mean, x_std, y_mean, y_std
    ...
    try:
        model.load_state_dict(torch.load("ns_inverse_model.pth", map_location=device))
    except FileNotFoundError:
        print("Error: ns_inverse_model.pth not found.")
        return
    model.eval()
    ...
    # Create tensors
    # t is constant for this snapshot
    ...
    # Predict Pressure
    # We don't need the full 'physics' function here, just the model forward pass
    # Output 0 is Psi, Output 1 is Pressure
    with torch.no_grad():
    ...
    # Post-Processing
    # Pressure in incompressible flow is relative.
    # Align the means to make a valid visual comparison.
